...
fn handle_camera_movement(
    ...
    camera_transform.translation.x += if keyboard.pressed(KeyCode::ArrowUp) {-camera_speed} else {0.};
    camera_transform.translation.x += if keyboard.pressed(KeyCode::ArrowDown) {camera_speed} else {0.};
    camera_transform.translation.z += if keyboard.pressed(KeyCode::ArrowRight) {-camera_speed} else {0.};
    camera_transform.translation.z += if keyboard.pressed(KeyCode::ArrowLeft) {camera_speed} else {0.};
    let max_translation = 40.*Vec3::ONE;
    camera_transform.translation = (camera_transform.translation+0.5*max_translation).clamp(Vec3::ZERO, max_translation)-0.5*max_translation;
